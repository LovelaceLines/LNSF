...
export const transformDateTime = (v: string): string => {
  ...
  return `${yyyy}-${mm.toString().padStart(2, "0")}-${dd.toString().padStart(2, "0")}T${hh
    .toString()
    .padStart(2, "0")}:${mi.toString().padStart(2, "0")}:00`;
};

export const dateTimeSchema = z.string().transform((v) => transformDateTime(v));
