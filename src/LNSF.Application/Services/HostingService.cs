...
public class HostingService : IHostingService
{
    ...
    public async Task<Hosting> Create(Hosting hosting)
    {
        ...
        if (await _hostingRepository.ExistsWithDateConflict(hosting)) throw new AppException("Já existe uma hospedagem para este paciente neste período", HttpStatusCode.Conflict);
