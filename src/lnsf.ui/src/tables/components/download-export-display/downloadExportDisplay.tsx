...
import { exportDataToCSV, exportDataToJSON, exportDataToPDF, exportDataToXLSX } from "@/libs";
...
export const DownloadExportDisplay = (props: DownloadExportDisplayProps) => {
  ...
  const handleFormat = (format: "JSON" | "CSV" | "XLSX" | "PDF") => {
    ...
    switch (format) {
      case "JSON":
        exportDataToJSON(fileName, headId, rows);
        break;
      case "CSV":
        exportDataToCSV(fileName, headId, rows);
        break;
      case "XLSX":
        exportDataToXLSX(fileName, headValue, rows);
        break;
      case "PDF":
        exportDataToPDF(fileName, headValue, rows);
        break;
    }
  };
      ...
      <>
        ...
        <FormControl>
          <RadioGroup row value={rowsMode} onChange={handleRowsModeChange}>
            ...
            <FormControlLabel value="selected" control={<Radio />} label="Somente as selecionadas" />
          </RadioGroup>
        </FormControl>
        <Divider />
        ...
        <Box display="flex" justifyContent="center" gap={1}>
          ...
          <Button variant="contained" size="small" onClick={() => handleFormat("CSV")}>
            CSV
          </Button>
